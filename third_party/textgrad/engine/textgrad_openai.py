...
def _resolve_cache_root() -> str:
    """
    Determine a writable cache directory. Prefer TEXTGRAD_CACHE_DIR, then the
    user cache, and finally fall back to a local .cache/textgrad folder.
    """
    env_override = os.getenv("TEXTGRAD_CACHE_DIR")
    if env_override:
    ...
    try:
        ...
        fallback_root = os.path.join(os.getcwd(), ".cache", "textgrad")
...
class ChatOpenAI(EngineLM, CachedEngine):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        if openai_key:
            ...
            raise ValueError(
                "Please set OPENAI_API_KEY or OPENROUTER_API_KEY/API_KEY to run TextGrad models."
            )
    ...
    def _generate_from_single_prompt(
        ...
        try:
            ...
            logger.warning("ChatOpenAI falling back to offline response: %s", exc)
    ...
    def _generate_from_multiple_input(
        ...
        try:
            ...
            logger.warning("ChatOpenAI falling back to offline response: %s", exc)
            response_text = self._offline_response(json.dumps(formatted_content))
        self._save_cache(cache_key, response_text)
        return response_text

    def _offline_response(self, prompt_preview: str) -> str:
        snippet = " ".join(prompt_preview.split())[:200]
        return (
            f"[offline-textgrad] Unable to contact {self.model_string}. "
