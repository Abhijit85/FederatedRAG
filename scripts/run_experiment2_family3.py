...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "artifacts" / "verification" / "experiment2_family3"
DEFAULT_ANALYSIS_INPUT = REPO_ROOT / "artifacts" / "verification" / "experiment2_family3_seeds.json"
...
NEIGHBOR_STRENGTH = {
    ...
    20: {"typed": 0.08, "flat": 0.11},
    40: {"typed": 0.12, "flat": 0.17},
    60: {"typed": 0.16, "flat": 0.23},
}

TYPED_CONFLICT_REP_PROB = {
    0: 0.0,
    20: 0.10,
    40: 0.18,
    60: 0.24,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Near-miss family3 reconstruction for Experiment 2.")
...
def build_neighbors(records: list[ScenarioRecord]) -> dict[str, list[ScenarioRecord]]:
    ...
    for record in records:
        ...
        ranked = sorted(candidates, key=lambda cand: overlap_score(record, cand), reverse=True)
        neighbors[record.scenario] = ranked[:3]
    return neighbors
    ...
    for record in records:
        ...
        queries.append(
            QueryCase(
                ...
                query=(
                    f"Which {record.tool} scenario handles {record.decisive_cue} tasks with {supportive}? "
                    f"Prefer the closest specialization, not just the broad domain."
                ),
            )
        )
        queries.append(
            QueryCase(
                ...
                exemplar=record.exemplar,
                query=(
                    f"In {record.tool}, which scenario fits an example about {record.exemplar} and {record.decisive_cue}? "
                    f"Choose the specialization whose cues best align."
                ),
            )
        )
    return queries
...
def contradictory_artifact(record: ScenarioRecord, wrong: ScenarioRecord, source_id: str, condition: str, rate: int, rng: random.Random) -> Artifact:
    ...
    strength = NEIGHBOR_STRENGTH[rate][condition]
    if condition == "typed":
        if rng.random() < strength * 0.75:
...
def representative_artifact(cluster: list[Artifact], prefer_clean: bool, conflict_rate: int, rng: random.Random) -> Artifact:
    clean = [artifact for artifact in cluster if not artifact.metadata.get("conflicted")]
    conflicted = [artifact for artifact in cluster if artifact.metadata.get("conflicted")]
    if prefer_clean and clean:
        if conflicted and rng.random() < TYPED_CONFLICT_REP_PROB.get(conflict_rate, 0.0):
            return max(conflicted, key=lambda artifact: (len(artifact.text), len(artifact.payload)))
        return max(clean, key=lambda artifact: (len(artifact.text), len(artifact.payload)))
    return max(cluster, key=lambda artifact: (len(artifact.text), len(artifact.payload)))


def merge_cluster(cluster: list[Artifact], condition: str, conflict_rate: int, seed: int) -> MergedArtifact:
    rep_rng = random.Random((seed * 10000) + (conflict_rate * 97) + len(cluster))
    rep = representative_artifact(cluster, prefer_clean=(condition == "typed"), conflict_rate=conflict_rate, rng=rep_rng)
    if condition == "typed":
...
def retrieval_score(query_case: QueryCase, artifact: MergedArtifact) -> float:
    score = 0.0
...
def evaluate_condition(records: list[ScenarioRecord], queries: list[QueryCase], neighbors: dict[str, list[ScenarioRecord]], rate: int, seed: int, condition: str) -> dict[str, Any]:
    ...
    merged = [merge_cluster(cluster, condition, rate, seed) for cluster in cluster_artifacts(artifacts)]
    rows: list[dict[str, Any]] = []
    correct = 0
    for query_case in queries:
        ranked = sorted(merged, key=lambda artifact: retrieval_score(query_case, artifact), reverse=True)
        top = ranked[0]
        ...
        rows.append({
            ...
            "top_score": retrieval_score(query_case, top),
        })
